...
class DynamicCss
{
    ...
    public function build()
    {
        ...
        switch ($this->current) {
            case 'neuralink':
                $dynamic_css = $this->themeNeuralink();
                break;
        }

        $path = "css/" . $this->current . "-css.css";
        if (trim($dynamic_css) != ':root {}') {
    ...
    /*
    * Neuralink Theme dynamic CSS
    *
    *
    */
    private function themeNeuralink()
    {
        $dynamicCSS  = ":root {";
        ...
        if (!$this->is_theme_default('background_color', 'light')) {
            $dynamicCSS .= "
                            ...
                            --{$this->prefix}body-color: {$this->theme->light->background_color};
                        ";
        }
