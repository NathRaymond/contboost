...
Breadcrumbs::for('admin.pages', function (BreadcrumbTrail $trail) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.managePages'), route('admin.pages'), ['container' => 'container-fluid']);
});
...
Breadcrumbs::for('admin.pages.edit', function (BreadcrumbTrail $trail, Page $page) {
    $trail->parent('admin.pages');
    $trail->push(__('admin.editPage'), route('admin.pages.edit', $page), ['container' => 'container-fluid']);
});

// Posts Breadcrumbs
Breadcrumbs::for('admin.posts', function (BreadcrumbTrail $trail) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.managePosts'), route('admin.posts'), ['container' => 'container-fluid']);
});
...
Breadcrumbs::for('admin.tags', function (BreadcrumbTrail $trail) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.manageTags'), route('admin.tags'), ['container' => 'container-fluid']);
});
Breadcrumbs::for('admin.tags.edit', function (BreadcrumbTrail $trail, Tag $tag) {
    $trail->parent('admin.tags');
    $trail->push($tag->name, route('admin.tags.edit', $tag), ['container' => 'container-fluid']);
});

// Categories
Breadcrumbs::for('admin.categories', function (BreadcrumbTrail $trail, $type = null) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.manageCategories'), route('admin.categories', ['type' => $type]), ['container' => 'container-fluid']);
});
Breadcrumbs::for('admin.categories.edit', function (BreadcrumbTrail $trail, Category $category) {
    $trail->parent('admin.categories');
    $trail->push($category->name, route('admin.categories.edit', $category), ['container' => 'container-fluid']);
});

// users
Breadcrumbs::for('admin.users', function (BreadcrumbTrail $trail) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.users'), route('admin.users'), ['container' => 'container-fluid']);
});

// roles
Breadcrumbs::for('admin.roles', function (BreadcrumbTrail $trail, Role $role = null) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.roles'), route('admin.roles'), ['container' => 'container-fluid']);

    if ($role) {
        $trail->push($role->name, route('admin.roles', $role), ['container' => 'container-fluid']);
    }
});
...
Breadcrumbs::for('admin.plans.edit', function (BreadcrumbTrail $trail, Plan $plan) {
    $trail->parent('admin.plans');
    $trail->push($plan->name, route('admin.plans.edit', $plan), ['container' => 'container-fluid']);
});
...
// usecase
Breadcrumbs::for('admin.usecases', function (BreadcrumbTrail $trail) {
    $trail->parent('admin.dashboard');
    $trail->push(__('admin.usecases'), route('admin.usecases'), ['container' => 'container-fluid']);
});
Breadcrumbs::for('admin.usecases.create', function (BreadcrumbTrail $trail) {
    $trail->parent('admin.usecases');
    $trail->push(__('admin.usecasesCreate'), route('admin.usecases.create'), ['container' => 'container-fluid']);
});
Breadcrumbs::for('admin.usecases.edit', function (BreadcrumbTrail $trail, Usecase $usecase) {
    $trail->parent('admin.usecases');
    $trail->push($usecase->name, route('admin.usecases.edit', $usecase), ['container' => 'container-fluid']);
});
...
Breadcrumbs::for('tool.show', function (BreadcrumbTrail $trail,  Tool $tool) {
    $trail->parent('front');
