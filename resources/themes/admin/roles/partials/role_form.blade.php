<form action="{{ isset($roleEdit) ? route('admin.roles.update', $roleEdit) : route('admin.roles.store') }}"
    ...
    <div class="form-check form-switch mb-3">
        <input class="form-check-input" id="guest" type="checkbox" @if (isset($roleEdit) && $roleEdit->guest == 1) checked @endif
            name="guest" value="1">
        <label class="form-check-label" for="guest">@lang('admin.guest')</label>
        <br><span class="text-muted small">@lang('admin.guestHelpText')</span>
    </div>
